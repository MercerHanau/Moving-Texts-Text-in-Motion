...
void setup() {
  ...
  background(255);
  noStroke();
  frameRate(10);  // Limits speed from over-optimization. Set to above 60 (or 30 for vid) when you want it to go faster than one change each frame (frameCount can't be fractional)
...
// ALPHA CONTROL: for fade  
  float alphaControl = map(cc[16],0,127,0,255);  // reduced from 255 to 100 because of faster refresh rate
...
// SPEED CONTROL: how often to change the word (if (frameCount % speed == 0...)) 
  int speedControl = round(map(cc[17],0,127,200,10));  // Shouldn't be float b/c % 0
// --------------------
