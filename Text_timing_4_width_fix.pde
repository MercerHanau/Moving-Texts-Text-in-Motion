...
void draw() {
  ...
  float maxSizeW = fontSize/textWidth(words[index]) * (width-20);  
  ...
  // Draw word to screen, cycle through text. Size depends on if it falls out of bounds
  if (textWidth(words[index]) <= (width-20))   // if the text falls neatly within width bounds
  {
    text(words[index], width/2, (height/2)+30);  // Places text in middle of window.
  }
  else   // if wider than window, make small enough to fit
  {
    textSize(maxSizeW);  // Width of string depends on width of frame.
    text(words[index], width/2, (height/2)+30);
  }
  ...
  // Move to be in bottom left corner on any scale? Depends on string length and screen size.
  // Center should be at [(textWidth(str)/2) + arbitrary space constant] from left
  textSize(18);
  text(millis()/1000 + " seconds have passed since the start", 250,50);  
  text("you are in frame: " + frameCount,250,85);  // make these into strings so their lengths can be returned
